...
2. 素材名尽量四个字，最多不超过五个字。
3. 支持 hair 文件夹内前发/后发：
   - xxx_front.gif 进入 hair
   - xxx_back.gif 进入 hair_back
4. 读取并维护 素材中文命名表_四字版.json。
5. 新增素材会自动生成四字诗意名，已有名字会保留。
"""
...
ITEMS_ORDER = [
    "background", "front", "wing", "hair_back", "clothes", "bottom", "top",
    "body", "neck", "hair", "hat", "glasses", "earring", "face", "hand",
    "effect", "partner", "text", "frame"
]

COLOR_WORDS = {'white': ['月白', '雪瓷', '云白', '珍珠', '霜羽', '象牙', '银雾', '素雪', '玉色', '白露'], 'pink': ['樱粉', '桃绯', '蔷薇', '莓粉', '胭脂', '糖粉', '绯樱', '玫粉', '粉雾', '桃雪'], 'red': ['玫瑰', '朱砂', '绛红', '红茶', '石榴', '暮莓', '赤霞', '枫红', '红绒', '绯月'], 'orange': ['橘灯', '焦糖', '暖橙', '蜜杏', '夕橙', '南瓜', '琥珀', '杏茶', '橙花', '蜜橘'], 'yellow': ['鹅黄', '奶油', '金铃', '柠黄', '麦穗', '蜜金', '星砂', '金柠', '浅金', '月黄'], 'green': ['薄荷', '青柠', '森绿', '豆蔻', '翠羽', '青苔', '竹影', '青萝', '绿萤', '草木'], 'cyan': ['海盐', '水色', '湖青', '冰蓝', '浅潮', '玻璃', '晴川', '青瓷', '水雾', '蓝潮'], 'blue': ['天蓝', '雾蓝', '鲸蓝', '星河', '海湾', '蓝莓', '夜航', '晴蓝', '蓝梦', '冰海'], 'purple': ['藤紫', '鸢尾', '紫藤', '暮紫', '丁香', '星葡', '紫雾', '莓紫', '月紫', '紫晶'], 'brown': ['栗子', '可可', '茶棕', '咖啡', '榛果', '木樨', '焦栗', '奶茶', '松木', '棕糖'], 'gray': ['银灰', '雾灰', '烟蓝', '月影', '冷银', '铅云', '灰羽', '雾银', '石灰', '烟雨'], 'black': ['墨夜', '黑曜', '乌木', '夜雾', '鸦羽', '暗星', '玄月', '黑糖', '墨蓝', '夜幕'], 'colorful': ['虹光', '流彩', '糖果', '万花', '幻彩', '星爆', '霓虹', '彩梦', '珠光', '幻糖']}
...
MOTIFS = ['玻璃', '星屑', '花窗', '邮票', '糖纸', '胶片', '珍珠', '丝带', '月桂', '风铃', '贝壳', '烟火', '唱片', '纸鹤', '蝴蝶', '猫眼', '水晶', '云朵', '香槟', '琉璃', '旧信', '绒球', '流苏', '玫瑰', '小熊', '兔耳', '银链', '花束', '星轨', '雪绒', '海盐', '汽水', '绸缎', '花园', '舞会', '剧院', '梦境', '礼盒', '橱窗', '电波', '闪片', '徽章', '蕾丝', '羽毛', '果冻', '蜜糖', '樱桃', '蓝莓', '夜航', '童话', '雨伞', '信笺', '香水', '塔楼', '旋律', '烟波', '软糖', '冰沙', '烛光', '魔法', '航线', '书页', '雪松', '露珠', '日记', '星灯', '霓虹', '茶会', '花火', '银河', '洛可', '香芋', '缎面', '星云', '月轮', '花火', '果茶', '烫金', '水母', '甜筒']
STYLE_WORDS = ['复古', '甜酷', '学院', '梦幻', '奶油', '哥特', '童话', '港风', '森系', '海盐', '洛丽', '电波', '古典', '轻盈', '闪耀', '软糯', '俏皮', '清冷', '华丽', '日常', '舞会', '假日', '漫画', '偶像', '雪国', '糖果', '暗黑', '花园', '剧场', '未来']
CATEGORY_NOUNS = {'background': ['房间', '街角', '花园', '剧场', '窗景', '海岸', '城堡', '小屋', '月台', '天幕', '糖铺', '茶室', '舞厅', '车站', '露台', '阁楼', '游园', '星港', '梦廊', '书房', '庭院', '灯街', '回廊', '幻境', '客厅', '沙龙', '店铺', '云阶', '夜市', '画室'], 'front': ['前景', '帘幕', '花枝', '云边', '光斑', '门框', '花影', '窗纱', '边饰', '屏风', '落花', '雪幕', '灯串', '星帘', '梦雾', '藤蔓', '礼花', '水纹', '前帘', '光幕'], 'wing': ['羽翼', '蝶翼', '光翼', '星翼', '薄翼', '梦翼', '花翼', '云翼', '月翼', '幻翼', '白羽', '黑羽', '流光', '翅影', '星羽', '云翅'], 'hair': ['长发', '短发', '卷发', '马尾', '公主', '蓬发', '侧发', '波头', '发辫', '披发', '挑染', '刘海', '层发', '绒发', '娃发', '睡发', '漫发', '院发', '偶发', '发尾'], 'hair_back': ['后发', '发影', '后卷', '背发', '后束', '发片', '垂发', '后层', '发尾', '发帘'], 'clothes': ['礼裙', '套装', '衣裙', '制服', '舞裙', '洋装', '斗篷', '睡衣', '茶裙', '公主', '机车', '学院', '偶像', '魔裙', '哥裙', '旗袍', '水手', '外套', '派装', '旅装', '兔装', '熊装', '婚纱', '毛装', '运动', '侦探', '女仆', '乐队', '甜裙', '古裙'], 'top': ['上衣', '针织', '外套', '衬衫', '背心', '披肩', '毛衣', '夹克', '吊带', '卫衣', '罩衫', '蕾衫', '上装', '斗篷', '花衫', '院衫', '短袖', '长袖', '围肩', '纱衣'], 'bottom': ['半裙', '短裙', '长裙', '裤装', '灯裤', '褶裙', '纱裙', '瓜裤', '短裤', '花裙', '院裙', '蓬裙', '鱼裙', '底裙', '裙摆', '牛裙', '动裤', '小裙', '裙裤', '闲裤'], 'body': ['素身', '娃身', '基础', '站姿', '脸型', '默认', '微笑', '小脸', '肤色', '角色', '模特', '人偶', '清爽', '暖肤', '冷白'], 'neck': ['项链', '围巾', '颈饰', '领结', '锁链', '珠链', '丝巾', '铃铛', '颈带', '花环', '围脖', '披领', '吊坠', '绒围', '星链', '月链', '项圈', '短链', '缎带', '颈环'], 'hat': ['帽子', '礼帽', '发冠', '贝帽', '兔帽', '结帽', '皇冠', '花帽', '织帽', '发箍', '头饰', '鸭帽', '睡帽', '巫帽', '发带', '猫耳', '水帽', '王冠', '花冠', '发饰'], 'glasses': ['眼镜', '墨镜', '圆镜', '星镜', '框镜', '镜片', '护镜', '镜框', '心镜', '院镜', '古镜', '细镜', '透镜', '彩镜', '茶镜'], 'earring': ['耳坠', '耳环', '耳饰', '星耳', '珠坠', '蝶耳', '铃耳', '花耳', '晶坠', '月耳', '流苏', '糖耳', '圆坠', '银针', '心耳'], 'face': ['脸饰', '贴纸', '腮红', '面饰', '花贴', '星贴', '泪痣', '猫须', '心贴', '彩绘', '眼妆', '额饰', '雀斑', '表情', '亮片'], 'hand': ['手持', '花束', '雨伞', '玩偶', '权杖', '气球', '魔棒', '信封', '糖果', '手袋', '扇子', '礼物', '咖杯', '话筒', '小旗', '相机', '玫瑰', '书本', '星杖', '灯笼'], 'effect': ['特效', '光效', '星光', '泡泡', '闪粉', '花火', '落雪', '流光', '心光', '魔阵', '彩带', '烟花', '光环', '电波', '梦雾', '星雨', '旋光', '亮片', '光斑', '浮光'], 'partner': ['陪伴', '伙伴', '宠物', '玩偶', '精灵', '小熊', '小兔', '猫咪', '鸟儿', '小狗', '蘑菇', '云宠', '星伴', '花灵', '气球', '海豹', '狐狸', '鹿鹿', '机器', '幽灵'], 'text': ['字牌', '签名', '贴文', '标语', '弹幕', '话语', '告白', '闪字', '标题', '手写', '心情', '标识', '留言', '歌词', '问候', '注释', '横幅', '招牌', '字条', '短句'], 'frame': ['相框', '边框', '画框', '镜框', '胶框', '花框', '星框', '晶框', '窗框', '幕框', '信框', '宝框', '蕾框', '古框', '卡框', '银框', '糖框', '霓框', '透框', '念框']}
...
def display_category_for_rel(rel):
    parts = rel.split("/")
    cat = parts[0] if parts else "root"
    if cat == "hair" and is_hair_back(rel):
        return "hair_back"
    return cat
...
def candidate_names(cat, rel, path):
    ...
    nouns = CATEGORY_NOUNS.get(cat, ["素材"])
    key = src

    pools_a = [color_words, MOTIFS, MOODS, STYLE_WORDS]

    if bright_tag == "dark":
        pools_a.insert(0, ["墨夜","黑曜","夜雾","暗星","玄月","深海","月影","夜幕"])
    elif bright_tag == "light":
        pools_a.insert(0, ["月白","雪瓷","云白","晴窗","微光","春白","雪光","银雾"])

    if sat_tag == "vivid":
        pools_a.insert(0, ["霓虹","闪耀","糖果","流彩","星爆","幻彩","电波","花火"])
    elif sat_tag == "soft":
        pools_a.insert(0, ["奶油","轻盈","雾感","温柔","柔纱","旧梦","微光","云朵"])

    candidates = []

    for salt_i, pool in enumerate(pools_a):
    ...
    for j in range(16):
        a = MOODS[(stable_index(key, len(MOODS), "mood2") + j * 4) % len(MOODS)]
        b = nouns[(stable_index(key, len(nouns), "noun3") + j * 5) % len(nouns)]
        candidates.append(a + b)
...
def main():
    ...
    for p in all_files:
        ...
        src = "assets/" + rel
        cat = display_category_for_rel(rel)
    ...
    for src, nm in list(name_map.items()):
        if src.startswith("assets/hair/") and is_hair_front(src):
            hair_base_to_front_name[normalize_key_for_hair(src)] = nm

    for src in list(name_map.keys()):
        if src.startswith("assets/hair/") and is_hair_back(src):
            base = normalize_key_for_hair(src)
            if base in hair_base_to_front_name:
                proposed = hair_base_to_front_name[base] + "后"
    ...
    print(f"命名表：{NAME_MAP_FILE.name}")
